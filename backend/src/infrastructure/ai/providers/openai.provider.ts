...
export class OpenAiProvider implements IAiProvider {
  ...
  isConfigured(): boolean {
    ...
    return key !== "" && !key.includes("YOUR_OPENAI_API_KEY");
  }
  ...
  async generateSubtasks(
    ...
    if (!this.isConfigured()) {
      throw new ApiError(400, "AI_CONFIG_ERROR", "OPENAI_API_KEY chưa được cấu hình hoặc không hợp lệ");
    }
...
["Khởi tạo dữ liệu schema và migration", "Xây dựng RESTful API controller xử lý logic", "Tích hợp giao diện UI với API backend", "Viết Unit Test và kiểm thử luồng chạy"]`;
    ...
    const models = customModel ? [customModel] : ["gpt-4o-mini", "gpt-4o"];
    let lastErrorMsg = "";
    ...
    for (const model of models) {
      try {
        ...
        if (!response.ok) {
          ...
          console.error(`[OpenAiProvider] Model ${model} failed:`, response.status, errText);
        ...
        lastErrorMsg = err?.message || "Lỗi khi gọi API OpenAI";
      }
    }
    ...
    throw new ApiError(
      400,
      "AI_SERVICE_ERROR",
      `OpenAI Provider thất bại: ${lastErrorMsg}`
    );
  }
}
