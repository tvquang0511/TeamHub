...
export class TogetherProvider implements IAiProvider {
  ...
  isConfigured(): boolean {
    ...
    return key !== "" && !key.includes("YOUR_TOGETHER_API_KEY");
  }
  ...
  async generateSubtasks(
    ...
    if (!this.isConfigured()) {
      throw new ApiError(400, "AI_CONFIG_ERROR", "TOGETHER_API_KEY chưa được cấu hình hoặc không hợp lệ");
    }
...
["Khởi tạo dữ liệu schema và migration", "Xây dựng RESTful API controller xử lý logic", "Tích hợp giao diện UI với API backend", "Viết Unit Test và kiểm thử luồng chạy"]`;
    ...
    if (models.length === 0) {
      try {
        const resModels = await fetch("https://api.together.xyz/v1/models", {
          headers: { Authorization: `Bearer ${apiKey}` },
        });
        if (resModels.ok) {
          const dataModels = await resModels.json() as any;
          const availableModels = dataModels?.filter((m: any) => (m.id.includes("llama") || m.id.includes("mixtral")) && !m.id.includes("guard")).map((m: any) => m.id);
      ...
      if (models.length === 0) {
        models = ["meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo", "meta-llama/Meta-Llama-3-70B-Instruct-Turbo"];
      }
    }
    ...
    for (const model of models) {
      try {
        ...
        if (!response.ok) {
          ...
          console.error(`[TogetherProvider] Model ${model} failed:`, response.status, errText);
        ...
        lastErrorMsg = err?.message || "Lỗi khi gọi API Together AI";
      }
    }
    ...
    throw new ApiError(
      400,
      "AI_SERVICE_ERROR",
      `Together AI Provider thất bại: ${lastErrorMsg}`
    );
  }
}
